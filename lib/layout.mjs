...
/** Строка «правила плана» для шапок/легенд/экспортов.
 *  Классическая сетка → «1 стенд = 3 × 3 м = 9 м² · 1 блок = 8 стендов = 72 м²»;
 *  чертёжный план (площади разные) → «стенды 6–40 м² (по чертежу) · базовая ячейка 3×3 м · блоки повторяют разбивку чертежа».
 *  exp — результат expandLayout() или ответ /api/layout (нужны stands[] и meta). */
export function ruleNote(exp) {
  const stands = (exp && exp.stands) || [];
  const fmt = (n) => String(Math.round(n * 100) / 100).replace('.', ',');
  const uniq = [...new Set(stands.map((s) => Number(s.areaM2)).filter((x) => x > 0))].sort((a, b) => a - b);
  const byDrawing = !!(exp && exp.meta && exp.meta.enforceBlockRule === false);
  const blockPart = byDrawing ? 'блоки повторяют разбивку чертежа' : '1 блок = 8 стендов = 72 м²';
  if (!uniq.length || uniq.length === 1) return `1 стенд = 3 × 3 м = ${fmt(uniq[0] || 9)} м² · ${blockPart}`;
  const range = uniq.length > 4 ? `${fmt(uniq[0])}–${fmt(uniq[uniq.length - 1])}` : uniq.map(fmt).join('/');
