...
export function validateLayout(raw) {
  ...
  for (const b of gridBlocks) {
    ...
    if (!b.merged.length && b.stands.length !== 8) blockProblem(`${b.id}: ${b.stands.length} ta stend — standart blok 8 ta bo'lishi shart (72 m2 = 8 x 9)`);
    if (!near(totalArea, 72)) blockProblem(`${b.id}: maydon ${totalArea.toFixed(2)} m2 — 72 m2 bo'lishi shart${b.merged.length ? ` (${b.stands.length} stend + ${b.merged.length} birlashgan katak)` : ''}`);
    ...
    for (const m of b.merged) {
      ...
      if (!gridAligned(m.x - b.x, meta.stand.w) || !gridAligned(m.y - b.y, meta.stand.h) || !gridAligned(m.w, meta.stand.w) || !gridAligned(m.h, meta.stand.h)) {
        err(`${b.id}: birlashtirilgan "${name}" 3 m to'rga tekislanmagan (x=${m.x}, y=${m.y}, w=${m.w}, h=${m.h})`);
      }
      ...
      if (ov(mRect, blockRect)) err(`${b.id}: birlashtirilgan "${name}" stendlar ustiga tushgan`);
      const connected = touch(mRect, blockRect) || b.merged.some((o) => o !== m && touch(mRect, { x: o.x, y: o.y, w: o.w, h: o.h }));
      if (!connected) err(`${b.id}: birlashtirilgan "${name}" blokka tutashmagan (uzilib qolgan)`);
      for (const other of blocks) {
        if (other.id === b.id) continue;
        if (ov(mRect, { x: other.x, y: other.y, w: other.w, h: other.h })) err(`${b.id}: birlashtirilgan "${name}" ${other.id} bloki bilan kesishyapti`);
      }
    }
  }
  ...
  for (const m of mergedRects) {
    for (const s of stands) {
      if (inter(m, s) > 1e-4) err(`${m.id} (birlashgan katak) ${s.id} bilan kesishyapti`);
    }
  }
  ...
  for (const b of blocks) {
    if (b.section && !sectionIds.has(b.section)) err(`${b.id}: "${b.section}" bo'limi sections[] ro'yxatida yo'q`);
  }
  for (const b of blocks) {
    if (!b.section && (exp.sections || []).length) warn(`${b.id}: bo'lim (section) ko'rsatilmagan — xaritada bo'limsiz qoladi`);
  }
  for (const s of exp.sections || []) {
    const mine = blocks.filter((b) => b.section === s.id);
    if (!mine.length) warn(`"${s.label}" bo'limida birorta stend yo'q`);
  }
