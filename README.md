...
Если панель открыта внутри рамки (предпросмотр, встроенный просмотрщик), браузер обычно запрещает
скачивание файлов прямо в рамке — тогда кнопка открывает PDF **в новой вкладке** (`/api/export/pdf?disp=inline`),
а сохранить его можно кнопкой браузера. В отдельном окне (обычный браузер) файл скачивается сразу,
с готовым именем `FOODERA-EXPO-2026-plan-A3.pdf`.
Если и новую вкладку браузер блокирует, открывается окно **«PDF готов»**: в нём сам файл (просмотр),
ссылка на него, кнопка «Скопировать ссылку» и «Печать → Сохранить как PDF» — то есть PDF можно получить
даже там, где скачивание и вкладки запрещены полностью.
