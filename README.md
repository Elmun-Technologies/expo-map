...
**Печать / PDF:** кнопка `Печать плана` выводит **ровно одну страницу A3** — тот же готовый лист,
что уходит клиенту (`/api/export/svg`): шапка с проектом, датой и статистикой + сам план зала.
Поля листа нулевые, а чертёж вписан в 419×295 мм из 420×297 — поэтому браузер не может
...
В отдельном окне (обычный браузер) файл скачивается сразу, с готовым именем `FOODERA-EXPO-2026-plan-A3.pdf`.
...
| `GET /api/export/pdf` | готовый PDF карты одним листом A3 (кнопка «Скачать PDF»); `&disp=inline` — показать в браузере вместо скачивания |
| `GET /api/export/svg` | готовый лист A3 разметкой (для печати «одна страница» из панели; кешируется по revision) |
